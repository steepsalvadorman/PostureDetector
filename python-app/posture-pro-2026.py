"""
Evaluador ROSA — NTP 1173 (INSST, 2022)   v3.0
================================================
INSTALACIÓN:
...
EMPAQUETAR:
    ...
    pyinstaller --onefile --windowed --name ROSA_NTP1173 evaluacion_ROSA_NTP1173.py
"""
...
# ─────────────────────────────────────────────────────────────
C = {
    "bg_deep":   "#0A0C10",   # fondo profundo
    "bg_panel":  "#111318",   # paneles y cabeceras
    "bg_card":   "#181C22",   # tarjetas / superficies
    "bg_border": "#252A34",   # bordes sutiles
    "accent":    "#00C8FF",   # azul eléctrico — títulos y acento
    "accent2":   "#0087CC",   # azul oscuro — línea decorativa
    "text_hi":   "#F0F4FF",   # BLANCO CÁLIDO — texto principal, máxima legibilidad
    "text_mid":  "#9BAEC8",   # AZUL GRISÁCEO — valores numéricos / secundario
    "text_lo":   "#56657A",   # GRIS MEDIO — etiquetas / hints (más legible que antes)
    "ok":        "#00D68F",   # verde — score bajo / OK
    "warn":      "#F6AD3B",   # amarillo — score medio
    "danger":    "#FF4757",   # rojo — alerta / score alto
    "danger2":   "#CC1F2D",   # rojo oscuro — hover / alerta severa
}
...
def tlu(tabla, fi, ci):
    return int(tabla[int(np.clip(fi,0,tabla.shape[0]-1))][int(np.clip(ci,0,tabla.shape[1]-1))])
...
def calcular_ROSA(ang_tronco, desv_cuello, ang_rodilla, ang_codo, factor_t):
    ...
    score_B  = tlu(TABLA_B,p_B1-1,p_B2-1)
    p_C1 = 1 if 80<=ang_codo<=110 else 2
    p_C2 = 1 if 80<=ang_codo<=110 else 2
    score_C  = tlu(TABLA_C,p_C1-1,p_C2-1)
    score_D  = tlu(TABLA_D,score_B-1,score_C-1)
    rosa = int(np.clip(max(score_s,score_D),1,10))
    det = {
        "p_A1_altura":p_A1,"p_A2_profundidad":p_A2,
        "p_A3_reposabrazos":p_A3,"p_A4_respaldo":p_A4,
        "tabla_A":score_A,"factor_F":factor_t,"score_silla":score_s,
        "p_B1_telefono":p_B1,"p_B2_pantalla":p_B2,"tabla_B":score_B,
        "p_C1_raton":p_C1,"p_C2_teclado":p_C2,
        "tabla_C":score_C,"tabla_D":score_D,"rosa_final":rosa,
    }
...
# ─────────────────────────────────────────────────────────────
COLS_XLSX = [
    ("id_evaluado","id_evaluado"),("nombre_evaluado","nombre_evaluado"),
    ("horas_uso_diario","horas_uso_diario"),("timestamp","timestamp"),
    ("fecha","fecha"),("hora","hora"),("rosa_final","rosa_final"),
    ("nivel_riesgo","nivel_riesgo"),("alerta_disparada","alerta_disparada"),
    ("score_silla","score_silla"),("score_perifericos_D","score_perifericos_D"),
    ("p_A1_altura_asiento","p_A1_altura"),("p_A2_profundidad","p_A2_profundidad"),
    ("p_A3_reposabrazos","p_A3_reposabrazos"),("p_A4_respaldo","p_A4_respaldo"),
    ("tabla_A_silla","tabla_A"),("factor_tiempo_F","factor_F"),
    ("p_B1_telefono","p_B1_telefono"),("p_B2_pantalla","p_B2_pantalla"),
    ("tabla_B_tel_pant","tabla_B"),("p_C1_raton","p_C1_raton"),
    ("p_C2_teclado","p_C2_teclado"),("tabla_C_rat_tec","tabla_C"),
    ("tabla_D_perifericos","tabla_D"),("ang_tronco_deg","ang_tronco_deg"),
    ("ang_cuello_desv_deg","ang_cuello_desv_deg"),
    ("ang_rodilla_deg","ang_rodilla_deg"),("ang_codo_deg","ang_codo_deg"),
]
...
def exportar_xlsx(ruta, registros):
    ...
    if os.path.exists(ruta):
        ...
        for ci,col in enumerate(hdrs,1):
            ...
            c.border = thin()
        anchos = {"timestamp":17,"fecha":12,"hora":10,"nombre_evaluado":22,
                  "id_evaluado":14,"nivel_riesgo":20,"alerta_disparada":16,
...
class HiloCamara(threading.Thread):
    def __init__(self,cola_datos,cola_frames,factor_t,cam_idx):
        ...
        self.cola_datos=cola_datos; self.cola_frames=cola_frames
        self.factor_t=factor_t; self.cam_idx=cam_idx; self.activo=True
        ...
        while self.activo and cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if dist_h < 130:
                    ...
                    try:
                        ear=gp("EAR"); sh=gp("SHOULDER"); hip=gp("HIP")
                        kn=gp("KNEE"); ank=gp("ANKLE"); el=gp("ELBOW"); wr=gp("WRIST")
                        ...
                        for i,(lbl,val) in enumerate([
                            ...
                            cv2.putText(frame,f"{lbl}: {val:.0f}deg",
                                        (12,h-14-i*18),cv2.FONT_HERSHEY_SIMPLEX,
                                        0.44,(0,200,120),1)
                    except: pass
            ...
            if time.time()-inicio >= INTERVALO_SEG and len(buffer)>=3:
                ...
                rosa,ss,sd,det = calcular_ROSA(
                    float(avg[0]),float(avg[1]),float(avg[2]),float(avg[3]),self.factor_t)
...
class VentanaAlerta(tk.Toplevel):
    def __init__(self,parent,rosa,nombre,callback_cerrar=None):
        ...
        self._activa=True; self._callback=callback_cerrar; self._blink_id=None
...
# ─────────────────────────────────────────────────────────────
# PANTALLA DE INICIO — estética industrial, un solo botón
# ─────────────────────────────────────────────────────────────

def _style_combo_popup(cb):
    """Aplica colores oscuros al Listbox popup del Combobox en Windows."""
    try:
        cb.tk.eval(f"""
...
class PantallaInicio(tk.Toplevel):
    ...
    def _mk_combo(self,parent,values):
        ...
        sty.configure("D.TCombobox",
            ...
            background=C["bg_card"],
            foreground=C["text_hi"],
            insertcolor=C["text_hi"],
            arrowcolor=C["accent"],
            bordercolor=C["bg_border"],
        ...
        # Forzar colores del Listbox desplegable (el popup)
    ...
    def _build(self):
        ...
        tk.Label(hdr,text="SISTEMA AUTOMATIZADO EVALUACIÓN ROSA",bg=C["bg_panel"],fg=C["accent"],
    ...
    def _ok(self):
        ...
        for e,w,v in [(self.e_nombre,self._wrap_nombre,nombre),
            ...
            if not v:
                w.configure(highlightbackground=C["danger"],
                            highlightcolor=C["danger"])
                err = True
            else:
                w.configure(highlightbackground=C["bg_border"],
...
class PanelROSA(tk.Tk):
    ...
    def __init__(self,nombre,id_t,uso_horas,cam_idx):
        ...
        # Último resultado ROSA conocido — se reutiliza para registros cada 3 s
    ...
    def _build_camara(self):
        ...
        self.barra_cd = ttk.Progressbar(cd_row,
                                         style="Bar.Horizontal.TProgressbar",
                                         maximum=INTERVALO_SEG,
                                         length=self.CAM_W-110)
    ...
    def _build_panel(self):
        ...
        self.lbl_score = tk.Label(row_s,text="—",
        ...
        self.barra_rosa = ttk.Progressbar(cs,
                                           style="Bar.Horizontal.TProgressbar",
    ...
    def _guardado_periodico(self):
        """
        Cada GUARDADO_SEG segundos:
          1. Si hay un resultado ROSA conocido, añade UN registro nuevo con
             timestamp actualizado (aunque la evaluación no haya cambiado).
          2. Persiste todos los registros al XLSX.
        Así el archivo crece continuamente a razón de 1 fila / GUARDADO_SEG s.
        """
        try:
            if self._ultimo_resultado is not None:
                # Crear registro nuevo con timestamp del momento actual
    ...
    def _poll(self):
        try:
            ...
            img = Image.fromarray(rgb).resize(
                      (self.CAM_W,self.CAM_H),Image.LANCZOS)
        ...
        try:
            ...
            # Guardar último resultado para que _guardado_periodico lo use
    ...
    def _detener(self):
        ...
        # Forzar un último guardado con timestamp actual antes de cerrar
        if self._ultimo_resultado is not None:
            try:
                r = self._ultimo_resultado
                ts = time.strftime("%Y-%m-%d %H:%M:%S")
                nivel_txt, _ = nivel_accion(r["rosa"])
                self.registros.append({
                    "id_evaluado":self.id_t,"nombre_evaluado":self.nombre,
                    "horas_uso_diario":self.uso_horas,"timestamp":ts,
                    "fecha":ts[:10],"hora":ts[11:],"rosa_final":r["rosa"],
                    "nivel_riesgo":nivel_txt,
                    "alerta_disparada":"SI" if r["rosa"]>=NIVEL_ACCION else "NO",
                    "score_silla":r["ss"],"score_perifericos_D":r["sd"],
                    "p_A1_altura":r["detalle"].get("p_A1_altura",""),
                    "p_A2_profundidad":r["detalle"].get("p_A2_profundidad",""),
                    "p_A3_reposabrazos":r["detalle"].get("p_A3_reposabrazos",""),
                    "p_A4_respaldo":r["detalle"].get("p_A4_respaldo",""),
...
if __name__=="__main__":
    ...
    PanelROSA(
        nombre=datos["nombre"],id_t=datos["id"],
        uso_horas=datos["horas"],cam_idx=datos["cam_idx"],
    ).mainloop()